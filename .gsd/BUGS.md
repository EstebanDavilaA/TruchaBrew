...
- **Resolution Path:** **Deliberately none scheduled.** Logged so it is not rediscovered as a new finding, and so the next session that sees a single unexplained red test on an otherwise-green tree re-runs before treating it as a regression. **Standing instruction: capture the failing test name and output the next time it fires** — that single artifact converts this from an unactionable note into a diagnosable defect. Escalate to `/diagnose` if the rate rises, if it becomes reproducible, or if it starts landing on a Layer 1 gate during `/execute` or `/steer`. Milestone 42 — which carries this project's first CI workflow — is where this would surface most often and most cheaply, since CI runs the suite far more than a human does. *(Reference corrected 2026-09-03: originally cited Milestone 47 of the 2026-09-02 eight-milestone deployment draft, which was rescoped to four milestones; the CI scope moved into Milestone 42.)*
