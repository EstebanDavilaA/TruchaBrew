...
export const YeastSection: React.FC<YeastSectionProps> = ({ yeasts, onUpdate }) => {
  ...
  const handleAddFromCatalog = () => {
    const catalogItem = INGREDIENT_CATALOG.yeasts.find((y) => y.id === selectedCatalogId);
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-lg mb-6">
      ...
      <div className="overflow-x-auto mb-4">
        <table className="w-full text-left text-sm text-slate-300">
          <thead className="bg-slate-800/80 text-xs text-slate-400 uppercase border-b border-slate-700">
            <tr>
              <th className="py-2 px-3">Yeast Name</th>
              <th className="py-2 px-3">Laboratory</th>
              <th className="py-2 px-3">Type</th>
              <th className="py-2 px-3">Form</th>
              <th className="py-2 px-3 text-right">Attenuation %</th>
      ...
      <div className="flex flex-wrap items-center gap-3 bg-slate-800/50 p-3 rounded-lg border border-slate-800">
        ...
        <select
          ...
          className="flex-1 min-w-[220px] bg-slate-800 border border-slate-700 rounded px-3 py-1.5 text-sm text-slate-200 focus:outline-none focus:border-purple-500"
        >
          <option value="">-- Select Yeast Strain --</option>
          {INGREDIENT_CATALOG.yeasts.map((cat) => (
